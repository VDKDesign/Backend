...
@section('title')
    <title>VDK Design | Aanmaken widget | Niveau 3</title>
@stop
...
@section('breadcrumb')
    <ol class="breadcrumb">
        ...
        <li class="active"><a href="{!! URL::to('/') !!}/backend/new_sub_widget_3">Aanmaken widget | Niveau 3</a></li>
    </ol>
@stop
...
@section('content')
    <div class="form_page_editor">
        ...
        @endif
        {!! Form::open(array('url' => '/backend/new_sub_widget_3/create')) !!}
            <div class="box-header-sub">
                <h3 class="box-title">Algemene eigenschappen</h3>
            </div>
            <div class="form-group">
                <label>Subpagina</label>
                <select class="form-control select2" name="subSubpagina" style="width: 100%;" >
                    ...
                    @foreach($menu_items as $value)
                        ...
                        @foreach($value->menu_sub_items as $subValue)
                            <option value="{{$subValue->id}}" disabled="disabled" >{{$subValue->title}}</option>
                            @foreach($subValue->menu_sub_sub_items as $subSubValue)
                                <option value="{{$subSubValue->id}}" @if(isset($id)){{ $id == $subSubValue->id ? 'selected="selected"' : '' }}@endif >{{$subSubValue->title}}</option>
                            @endforeach
                        @endforeach
                    @endforeach
                </select>
